...
#define LED_READY D0  // LED 1 (Sistema Pronto)
#define LED_CARD_READ D3  // LED 2 (Cartão Lido)
#define LED_API_SUCCESS D8  // LED 3 (Comunicação com a API Bem-Sucedida)
...
void setup() {
  ...
  pinMode(LED_API_SUCCESS, OUTPUT);
  ...
  // Acende o LED de Sistema Pronto
...
void loop() {
  ...
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
    ...
    // Apaga o LED de Cartão Lido após a leitura
...
void sendJsonToApi(String rfid) {
  if (WiFi.status() == WL_CONNECTED) {  // Verifica se está conectado ao Wi-Fi
    ...
    if (httpResponseCode > 0) {
      ...
      // Acende o LED de Comunicação com a API Bem-Sucedida
      digitalWrite(LED_API_SUCCESS, HIGH);
      delay(1000);
      digitalWrite(LED_API_SUCCESS, LOW);
    } else {
    ...
    // Apaga o LED de Comunicação com a API Bem-Sucedida após a resposta
    digitalWrite(LED_API_SUCCESS, LOW);
  } else {
    Serial.println("Wi-Fi desconectado");
  }
